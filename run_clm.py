...
class KNNArguments:
    ...
    log_knn: bool = field(default=False)
    ratio: float = field(default=0)
...
def main():
    ...
    if not training_args.do_train:
        wandb.run.name = f"k={knn_args.k}, knn_temp={knn_args.knn_temp}"
    else:
        wandb.run.name = f"Train set: {data_args.train_file}"
    config = knn_args
    ...
    if training_args.do_eval:
        ...
        if knn_wrapper is not None and not (
            knn_args.save_knnlm_dstore or knn_args.build_index
        ):
            knn_metrics, knn_dists = knn_wrapper.get_metrics()
            knn_dists["correct"] = {
                k: np.mean(v) for k, v in knn_dists["correct"].items()
            }
            knn_dists["wrong"] = {k: np.mean(v) for k, v in knn_dists["wrong"].items()}

            res = pd.DataFrame.from_dict(
                {f"{k}": [list(v.values())] for k, v in knn_metrics.items()}
            )
            res_dist = pd.DataFrame.from_dict(knn_dists)
            mixed_key = False
            mixed_query = False
            if "ratio" in knn_args.dstore_dir:
                mixed_key = True
            if "ratio" in model_args.model_name_or_path:
                mixed_query = True

            res.index = [f"mixed_query={mixed_query}, mixed_key={mixed_key}"]
            if os.path.exists(f"knn_results/ratio={knn_args.ratio}.csv"):
                res_df = pd.read_csv(
                    f"knn_results/ratio={knn_args.ratio}.csv", index_col=0
                )
                res_df = res_df.append(res)
                res_df.to_csv(f"knn_results/ratio={knn_args.ratio}.csv")
            else:
                res.to_csv(f"knn_results/ratio={knn_args.ratio}.csv")
            # save dists

            res_dist.to_csv(
                f"knn_results/ratio={knn_args.ratio}_dist_mixed_query={mixed_query}_mixed_key={mixed_key}.csv"
            )
