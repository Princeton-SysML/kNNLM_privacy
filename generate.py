...
from utils import parse_commoncrawl
...
class ATTArguments:
    ...
    prompt_config: str = field(default="gpt-vocab")
    diversity_penalty: float = field(
        default=0.0
    )  # [0.25, 0.5, 0.75] This value is subtracted from a beam’s score if it generates a token same as any beam from other group at a particular time.
    repetition_penalty: float = field(
        default=1.0
    )  # [0.5, 0.75] The parameter for repetition penalty (multiply on the repeated token to reduce its probability). 1.0 means no penalty
    no_repeat_ngram_size: int = field(
        default=0
    )  # [0, 1, 2] If set to int > 0, all ngrams of that size can only occur once.
...
def main():
    ...
    wandb.init(project="Privacy-kNN-LM")
    wandb.run.name = f"[Generate], k={knn_args.k}, n_beam={attack_args.beam_size}"
    config = knn_args
    wandb.config.update(knn_args)
    ...
    if attack_args.do_generate:
        max_len = 35
        model.config.pad_token_id = model.config.eos_token_id
        if attack_args.prompt_config == "gpt-advance":
            import pandas as pd

            prompt_res = pd.read_csv("./prompt_list.csv")
            prompt_list = prompt_res["word"].tolist()
            n_generate_list = prompt_res["n_generate"].tolist()
        elif attack_args.prompt_config == "gpt-crawl":
            cc = parse_commoncrawl("./prompts/crawl/commoncrawl.warc.wet")
            prompt_list = []
            n_generate_list = None
            ...
            # max_len = 64
            max_len = 50
        elif attack_args.prompt_config == "sens-adaptive":
            prompt_list = []
            for sens_type in ["emails", "phones", "urls"]:
                prompt_list += [
                    l.strip()
                    for l in open(f"./prompts/{sens_type}.txt", "r").readlines()
                ]
            max_len = 64
            n_generate_list = [50] * len(prompt_list)

        if "checkpoint" in model_args.model_name_or_path:
            model_dir = model_args.model_name_or_path.split("/")[-1]
        else:
            model_dir = model_args.model_name_or_path
        if not os.path.exists(f"{training_args.output_dir}/{model_dir}"):
            os.makedirs(f"{training_args.output_dir}/{model_dir}")

        model_ref = transformers.GPT2LMHeadModel.from_pretrained(
            "gpt2", return_dict=True
        ).to(model.device)

        if knn_args.knn:
            dstore_dir = knn_args.dstore_dir.split("/")[-1]
            if not os.path.exists(
                f"{training_args.output_dir}/{model_dir}/{dstore_dir}"
            ):
                os.makedirs(f"{training_args.output_dir}/{model_dir}/{dstore_dir}")
            if not os.path.exists(
                f"{training_args.output_dir}/{model_dir}/{dstore_dir}/k={knn_args.k}_lambda={knn_args.lmbda}"
            ):
                os.makedirs(
                    f"{training_args.output_dir}/{model_dir}/{dstore_dir}/k={knn_args.k}_lambda={knn_args.lmbda}"
                )
            if not os.path.exists(
                f"{training_args.output_dir}/{model_dir}/{dstore_dir}/k={knn_args.k}_lambda={knn_args.lmbda}/{attack_args.prompt_config}"
            ):
                os.makedirs(
                    f"{training_args.output_dir}/{model_dir}/{dstore_dir}/k={knn_args.k}_lambda={knn_args.lmbda}/{attack_args.prompt_config}"
                )
            target_file = f"{training_args.output_dir}/{model_dir}/{dstore_dir}/k={knn_args.k}_lambda={knn_args.lmbda}/{attack_args.prompt_config}/bs={attack_args.beam_size}_diversity_penalty_{attack_args.diversity_penalty}_repetition_penalty_{attack_args.repetition_penalty}_no_repeat_ngram_size_{attack_args.no_repeat_ngram_size}.csv"
            print(target_file)
        else:
            if knn_args.replug:
                if not os.path.exists(f"{training_args.output_dir}/{model_dir}/replug"):
                    os.makedirs(f"{training_args.output_dir}/{model_dir}/replug")
                if not os.path.exists(
                    f"{training_args.output_dir}/{model_dir}/replug/{attack_args.prompt_config}"
                ):
                    os.makedirs(
                        f"{training_args.output_dir}/{model_dir}/replug/{attack_args.prompt_config}"
                    )
            else:
                if not os.path.exists(f"{training_args.output_dir}/{model_dir}/k=0"):
                    os.makedirs(f"{training_args.output_dir}/{model_dir}/k=0")
                if not os.path.exists(
                    f"{training_args.output_dir}/{model_dir}/k=0/{attack_args.prompt_config}"
                ):
                    os.makedirs(
                        f"{training_args.output_dir}/{model_dir}/k=0/{attack_args.prompt_config}"
                    )

            target_file = f"{training_args.output_dir}/{model_dir}/k=0/{attack_args.prompt_config}/bs={attack_args.beam_size}_diversity_penalty_{attack_args.diversity_penalty}_repetition_penalty_{attack_args.repetition_penalty}_no_repeat_ngram_size_{attack_args.no_repeat_ngram_size}.csv"

        prompt_list = prompt_list[:5000]
        with open(target_file, "w") as f:
            for ip, prompt in tqdm(enumerate(prompt_list), total=len(prompt_list)):
                ...
                # continue
                # if ip % 100 ==0:
                # print(ip, prompt, len(prompt_list))
                if n_generate_list is not None:
                    n_generate = min(n_generate_list[ip], 30)
                else:
                    n_generate = 10
                # try:
                for _ in range(n_generate // 10):
                    generated = model.generate(
                        ...
                        num_beams=attack_args.beam_size,
                        num_return_sequences=10,
                        do_sample=True,
                        return_dict_in_generate=True,
                        output_scores=True,
                        repetition_penalty=attack_args.repetition_penalty,
                        no_repeat_ngram_size=attack_args.no_repeat_ngram_size,
                        max_length=max_len,
                    )

                    # https://huggingface.co/docs/transformers/v4.22.2/en/main_classes/text_generation#transformers.generation_utils.GenerationMixin.generate
                    # repetition_penalty
                    # no_repeat_ngram_size
                    generated_ids = generated["sequences"]
                    generated_scores = generated["sequences_scores"]
                    ...
                    for i, beam_output in enumerate(generated_ids):
                        ...
                        if knn_args.knn:
                            ppl = (
                                torch.exp(
                                    model(
                                        input_ids=generated_ids[i].unsqueeze(0),
                                        labels=generated_ids[i].unsqueeze(0),
                                    )[0]
                                )
                                .cpu()
                                .detach()
                            ...
                            ppl = (
                                torch.exp(
                                    model(generated_ids[i], labels=generated_ids[i])[0]
                                )
                                .cpu()
                                .detach()
                                .item()
                        ...
                        ppl_ref = (
                            torch.exp(
                                model_ref(generated_ids[i], labels=generated_ids[i])[0]
                            )
                            .cpu()
                            .detach()
                            .item()
                        ...
                        sent = sent.replace("\n", " ")
                        f.write(
                            f"{ip};; {i};; {sent};; {-generated_scores[i]};; {ppl};; {ppl_ref}\n"
                        )
                # except Exception as e:
                # print(f'Prompt #{ip}: {prompt} fail')
                # print(e)
    ...
    if knn_args.cluster_dstore:
        knn_wrapper.cluster_dstore(
            num_clusters=knn_args.num_clusters,
            sample_size=knn_args.sample_size,
            model=model,
        )
